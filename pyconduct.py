...
#################################################################################################
	...
	#self.sourcesBoundFunction = sourcesBoundPYC
                ...
                calcSourcesB(self.indCB, self.cellC, self.sc, self.sp, self.ndist, \
                             self.cvol, self.af, self.gamma, self.lamb, self.xf, self.yf, self.sourcesBoundFunction) 
                self.sc, self.sp = calcSourcesV(self.sc, self.sp, self.cvol, self.phi, self.xc, self.yc, self.sourcesVolFunction) 
                self.act, self.apt = \
...
@jit
def calcSourcesB(indCB, cellC, sc, sp, ndist, cvol, af, gamma, lamb, xf, yf, function):
    for i in range(len(indCB)):
        ind = indCB[i]       
        c1 = cellC[ind,0]
        idb = cellC[ind,2]
        idf = cellC[ind,3]        
        sci, spi = function(idb, c1, idf, ind, ndist, cvol, af, gamma, lamb, xf, yf)
        sc[c1] += sci
        sp[c1] += spi
    return sc, sp
...
@jit
def calcSourcesV(sc, sp, cvol, phi, xc, yc, function):
    for i in range(len(sc)):
        sci, spi = function(i, cvol[i], phi, xc, yc)
        sc[i] += sci
        sp[i] += spi
    return sc, sp
...
def faceVertexC(ncvx, ncvy, faceV):
    ...
    for j in range(ncvy):
        for i in range(ncvx + 1):
            ...
            #print(start + i, startD + i, startU + i)
        start += add
        startD += addD
        startU += addU
    return faceV 
...
def vertex(ncvx, ncvy, vertexC, vertexID):
    ...
    #Recorrer vertices sentido horario
    suma = 0
    count = 0
...
@jit
def sourcesBoundPYC(b, c, f, i, ndist, cvol, af, gamma, lamb, xf, yf):
    #- 3: N
    #- 1: S
    #- 2: E
    #- 4: W
...
@jit
def sourcesVolPYC(c, vol, phi, xc, yc):
    scv = 0.
    spv = 0.
    return scv, spv
